...
def volcano_plot_tissue(tissue, xname, yname, q, dfplot, dfindex, ax, label, col= 'b', a= .8):
    """
...
def explode(x, y, q, dfplot, dfindex, colors, **kwargs):
    ...
    colors= a vector of colors. Should have as many colors as 2 + no. of tissues
    ...
    if nvals > colors:
        raise ValueError('there must be as many colors as there are tissues +2!')
...
def volcano_plot_cool_genes(q, x, y, gene_name, genes, dfplot, dfgenes,
                            ax, colors, a= 1, dfplotgenes= 'ens_gene', **kwargs):
    """
    Plots all the tissue specific genes,i.e. all genes that appear in one and only
    one 'tissue'
    """
    ...
    for i, gname in enumerate(dfgenes[gene_name].unique()):
        selected_genes= (dfgenes[gene_name] == gname) #slice out desired genes
        ind= f(dfgenes[selected_genes][genes])
        sliced= dfplot[ind].dropna().copy()
        
        ngsig= len(sliced[(sliced[y] < q)][dfplotgenes].unique()) #no. genes showing up in assay
        tg= len(dfgenes[dfgenes[gene_name] == gname][genes].unique()) #no. of genes in dataset
        label= '{0} {1}= {2}, tot= {3}'.format(gname, r'$n_{sig}$', ngsig, tg)
        plt.gca().plot(sliced[x], -np.log10(sliced[y]), 'o', 
...
def explode_cool_genes(q, x, y, gene_name, genes, dfplot, dfgenes, colors, 
    ...
    ind1= (~dfplot[dfplotgenes].isin(dfgenes[genes])) & (dfplot[y] < q) #sig genes not in any given df
    ind2= (~dfplot[dfplotgenes].isin(dfgenes[genes])) & (dfplot[y] > q) #nonsig genes
    
    xnotsig= dfplot[ind2][x]
    ynotsig= dfplot[ind2][y]
    
    xsig= dfplot[ind1][x]
    ysig= dfplot[ind1][y]
    
    nnotsig= len(dfplot[ind2][dfplotgenes].unique())
    fig, ax= plt.subplots()
    plt.plot(xnotsig, -np.log10(ynotsig), 'o', \
    color=colors[0], ms=6, alpha= .15, label= r'not sig, not in other set $n$='+'{0}'.format(nnotsig))
    
    nsig= len(dfplot[ind1][dfplotgenes].unique())
